...
void removefromfile(ofstream &fileIn,ifstream &inFile,Name &names, Contact &contact);
...
int main()
{
   ...
   while(!inFile.eof()){ // does while not at the end of the file
        lastnameFinder(inFile,contact.names.lastName); // get last name
        firstnameFinder(inFile,contact.names.firstName); // get first name
        GetString(inFile,contact.phoneNumber); // get phone number
        GetAddress(inFile,contact.address); // get address

        if(isInFinder(contact.names.lastName,contact.names.firstName,contact.phoneNumber,contact.address, input, selection)){ // check if search is in string
            printer(contact.names.lastName, contact.names.firstName, contact.phoneNumber, contact.address); // print if search is in string
            i++; // count if this loop runs if not the user will be notified
            contact.names.lastName.clear(); // clears last name to prevent duplicates
            contact.names.firstName.clear(); // clears first name to avoid duplicates
...
void removefromfile(ofstream &fileIn,ifstream &inFile, Name &names, Contact &contact){
            ...
            contact.names.lastName.delete(); // clears last name to prevent duplicates
            contact.names.firstName; // clears first name to avoid duplicates
            contact.phoneNumber; // clears address
            contact.address; //clears address
