...
// Have a phonebook program be searchable for users and have it continue users can also add and delete from file and everything is written to an array
// 4 April 2017
...
void closeFile(ifstream &inFile, ofstream &fileIn,  Contact contacts[]);/*in closes file*/
...
void found(int i, string input);/*i*/
//precondition: search results
// postcondition: results can be deleted from file
void removefromfile(Name &names, Contact contacts, int p);
//precondition: program has run and unknown if the user wants to continue
//postcondition: the program runs
int cont();/*out*/
bool isInFinder2(string input, Contact contacts[MAX_CON],char selection, int p);
//precondition: strings are declared but not output
//postcondition: strings are output
void printer2(Contact contacts[MAX_CON], int p); // print results

string NormalizeString(string &inString); // normalizes string

void closeFile2(ifstream &inFile, ofstream &fileIn);
...
int main()
{
   ...
   while(!inFile.eof()){ // does while not at the end of the file
        for( p = 0; p < MAX_CON; p++ ){
        contacts[p].names.lastName=lastnameFinder(inFile,contact.names.lastName); // get last name
        contacts[p].names.firstName= firstnameFinder(inFile,contact.names.firstName); // get first name
        contacts[p].phoneNumber = GetString(inFile,contact.phoneNumber); // get phone number
        contacts[p].address = GetAddress(inFile,contact.address); // get address

        if(isInFinder2(input,contacts,selection, p)){ // check if search is in string
            contacts[p].names.lastName=lastnameFinder(inFile,contact.names.lastName); // get last name reset
            contacts[p].names.firstName= firstnameFinder(inFile,contact.names.firstName); // get first name reset
            contacts[p].phoneNumber = GetString(inFile,contact.phoneNumber); // get phone number reset
            contacts[p].address = GetAddress(inFile,contact.address); // address reset

                    printer2(contacts, p);// print results if they fit search critera
                    removefromfile(names,contacts[p], p);// opens delete function;
        }
    ...
    closeFile(inFile, fileIn, contacts);// close
    }
    else if (select == 'a'){
            addtofile(fileIn,inFile);
        closeFile2(inFile, fileIn);// close
    }
...
void closeFile(ifstream &inFile, ofstream &fileIn, Contact contacts[])
{
    for (int p =0; p < MAX_CON; p++){
        fileIn << contacts[p].names.lastName+ ','; // write in last name
        fileIn << contacts[p].names.firstName<< endl; // write in first name
        fileIn <<contacts[p].phoneNumber+ ' '; // write in  phone number
        fileIn<< contacts[p].address;  // write in addres
    }
...
void removefromfile(Name &names, Contact contacts[MAX_CON], int p){

            char answer1;

            cout << "Would you like to delete this contact? (y/n) " << endl; // give the user an option to delete
            cin >> answer1;

                    while (answer1 != 'y'&& answer1 != 'Y'&& answer1 != 'N' && answer1 != 'n' ){ // forces user to enter a valid choice
                        cout << "Invalid entry" << endl;
                        cin.clear();
                        cin.ignore(100,'\n');
                        cout<< "Please enter a valid choice: ";
        ...
        contacts[p].names.lastName.clear();
        contacts[p].names.firstName.clear();
        contacts[p].phoneNumber.clear();
        contacts[p].address.clear();


        }
    if (answer1 == 'n' ||answer1 ==  'N'){

        cout << contacts[p].names.lastName<< endl;
        cout << contacts[p].names.firstName << endl;
        cout << contacts[p].phoneNumber << endl;
        cout << contacts[p].address << endl;
        cout << "has not been deleted" << endl;
...
bool isInFinder2(string input, Contact contacts[MAX_CON],char selection, int p){
            // normalize all strings to make sure they are not case sensitive
            NormalizeString(input);
            NormalizeString(contacts[p].names.lastName);
            NormalizeString(contacts[p].names.firstName);
            NormalizeString(contacts[p].phoneNumber);
            NormalizeString(contacts[p].address);


    //sort out the choice based on user selection
    // making the bool true or false depending if the input is in the string
        if (selection == 'l' ||selection ==  'L'){
                if(contacts[p].names.lastName.find(input) != string::npos)
                    return true;
               else
                return false;
                 ...
                 if (contacts[p].phoneNumber.find(input) != string::npos)
                    return true;
                else
                    return false;
                ...
                if (contacts[p].names.firstName.find(input) != string::npos)
                    return true;
                else
                    return false;
                ...
                if(contacts[p].address.find(input) != string::npos)
                    return true;
                else
                    return false;
...
void printer2(Contact contacts[MAX_CON], int p){
        // prints results

    cout << "--------------------------------------------------------------------------"<<endl;
    cout <<"|"<< " last name: " << contacts[p].names.lastName << "|"<<endl;
    cout <<"|"<<" first name: " << contacts[p].names.firstName <<"|"<< endl;
    cout <<"|"<< " phone number: " << contacts[p].phoneNumber <<"|"<< endl;
    cout <<"|"<< " address: " << contacts[p].address <<"|"<< endl;
    cout << "---------------------------------------------------------------------------"<<endl;
